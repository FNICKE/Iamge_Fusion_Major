...
# From: https://github.com/Zhaozixiang1228/MMIF-EMMA
# License: Research use - see original repo
...
class Restormer_CNN_block(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.embed = nn.Conv2d(
            in_dim, out_dim, kernel_size=3, stride=1, padding=1, bias=False, padding_mode="reflect"
        )
        self.GlobalFeature = GlobalFeatureExtraction(dim=out_dim, num_heads=8)
        self.LocalFeature = LocalFeatureExtraction(dim=out_dim)
        self.FFN = nn.Conv2d(
            out_dim * 2, out_dim, kernel_size=3, stride=1, padding=1, bias=False, padding_mode="reflect"
        )
...
class GlobalFeatureExtraction(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor=1.0, qkv_bias=False):
        super(GlobalFeatureExtraction, self).__init__()
        self.norm1 = LayerNorm(dim, "WithBias")
        self.attn = AttentionBase(dim, num_heads=num_heads, qkv_bias=qkv_bias)
        self.norm2 = LayerNorm(dim, "WithBias")
        self.mlp = Mlp(in_features=dim, out_fratures=dim, ffn_expansion_factor=ffn_expansion_factor)
...
class Ufuser(nn.Module):
    def __init__(self):
        ...
        self.V_down1 = nn.Conv2d(channel[0], channel[0], 3, 2, 1, bias=False, padding_mode="reflect")
        self.V_down2 = nn.Conv2d(channel[1], channel[1], 3, 2, 1, bias=False, padding_mode="reflect")
        self.V_down3 = nn.Conv2d(channel[2], channel[2], 3, 2, 1, bias=False, padding_mode="reflect")

        self.I_down1 = nn.Conv2d(channel[0], channel[0], 3, 2, 1, bias=False, padding_mode="reflect")
        self.I_down2 = nn.Conv2d(channel[1], channel[1], 3, 2, 1, bias=False, padding_mode="reflect")
        self.I_down3 = nn.Conv2d(channel[2], channel[2], 3, 2, 1, bias=False, padding_mode="reflect")

        self.up4 = nn.Sequential(
            nn.ConvTranspose2d(channel[3], channel[2], 4, 2, 1, bias=False), nn.ReLU()
        )
        self.up3 = nn.Sequential(
            nn.ConvTranspose2d(channel[2], channel[1], 4, 2, 1, bias=False), nn.ReLU()
        )
        self.up2 = nn.Sequential(
            nn.ConvTranspose2d(channel[1], channel[0], 4, 2, 1, bias=False), nn.ReLU()
        )
